...
import { Search, Crown, ThumbsUp, HeartHandshake, Wrench, MessageCircle, Eye, PenLine, Loader2, X } from 'lucide-react';
import { apiService, type StorySummary } from '../services/api';
import { useTimezone } from '../contexts/TimezoneContext';
import { formatRelativeOrDate } from '../utils/datetime';

// 智慧故事 archive list: 本週精選 shelf + search + sort + paginated cards.
// Publicly readable; writing goes through the parent's onCompose.
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {featured && <Crown className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />}
      </div>
...
export default function StoryList({
  ...
  const load = useCallback(async (query: string, sortKey: SortKey, pageNum: number, append: boolean) => {
    if (append) setLoadingMore(true); else setLoading(true);
    try {
      const data = await apiService.getStories({ q: query || undefined, sort: sortKey, page: pageNum });
  ...
  const changeSort = (key: SortKey) => {
    setSort(key);
    setPage(1);
    load(q.trim(), key, 1, false);
  };

  const loadMore = () => {
    const next = page + 1;
    setPage(next);
    load(q.trim(), sort, next, true);
  };
          ...
          {stories.length === 0 ? (
            searching ? (
              <div className="py-10 text-center text-petal-ink-soft" data-testid="story-search-empty">
                <p className="text-sm">找不到和「{q.trim()}」有關的故事。</p>
                <button
                  type="button"
                  onClick={() => search('')}
              ...
              <div className="bg-white border border-petal-rule-soft rounded-2xl p-8 text-center" data-testid="story-empty-state">
                ...
                <p className="text-petal-ink font-medium">你們走過的難關，可能正是另一對伴侶需要的地圖</p>
                <p className="text-xs text-petal-ink-soft mt-1.5 leading-relaxed max-w-sm mx-auto">
                  依照引導模板寫下一段真實經歷：發生了什麼、怎麼修復、學到什麼。故事會匿名發表（除非你選擇顯示暱稱），幫助遇到同樣狀況的人。
                </p>
                <button
                  type="button"
