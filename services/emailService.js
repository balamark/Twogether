...
<body>
  ...
  async sendEventNotification({ senderName, recipientEmail, eventTitle = '', type, messageContent = '' }) {
    if (!this.isConfigured()) return;
    if (!recipientEmail) return;
    ...
    const meta = {
      ...
      event_ai_comment:     { emoji: '🧑‍⚕️', headline: 'AI 諮商師在事件中留言',     subject: `🧑‍⚕️ AI 諮商師在你們的事件中留言了` },
    ...
    const messageLabel = type === 'event_ai_comment' ? 'AI 諮商師的留言' : `${senderName || '你的伴侶'} ${labelSuffix}`;
    const safeMessageLabel = type === 'event_ai_comment' ? 'AI 諮商師的留言' : `${safeSender} ${labelSuffix}`;
