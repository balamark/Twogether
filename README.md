...
### 2026-07-05
- **劇本圖片載入大幅加速**: 劇本清單與創作市集的封面縮圖改用專屬小圖（原本直接載入約 2048px
  的原圖，現在是 512px 的輕量 webp，檔案小約 10–20 倍），並加上瀏覽器層級的延遲載入——
  捲動到才下載，且圖片快取延長為一年，重複瀏覽幾乎瞬間顯示。舊劇本的封面也已一併轉換。
- **首次開啟更快**: 伺服器現在常駐一個執行個體（原本閒置後會休眠，久未使用後第一次開啟要等
  好幾秒冷啟動），深夜或清晨打開 App 也能立即載入。
