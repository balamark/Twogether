...
async function uploadToSupabase(fileBuffer, fileName, mimeType, bucket = 'photos') {
  ...
  // Filenames are content-unique (uuid + timestamp) and never rewritten, so
  // tell Supabase to serve them with a year-long cache instead of its default
  // max-age=3600 — browsers stop re-downloading photos every hour.
  const headers = {
