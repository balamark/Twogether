...
function serializeEvent(row, extras = {}) {
  return {
    ...
    title: row.title,
    summary: row.summary,
    // 完整經過 — the full rewrite kept for long drafts, null for short ones.
...
function serializeMessage(row) {
  return {
    ...
    // Sophie 衝突即時介入: when a reply was released through the mediator, its
    // translation rides on the message so the partner sees the original + the
    // underlying need together, regardless of the 情緒翻譯 lens toggle.
    sophie_translation: row.sophie_translation || null,
    sophie_need: row.sophie_need || null,
    // Display name of the AI 諮商師 that helped compose it ("X 協助表達").
    sophie_companion: row.sophie_companion || null,
  };
}
