...
// True while the couple holds an unexpired entitlement flagged unlimited_ai
// (e.g. the repo-owner coupon) — overrides the icebreaker_per_day cap to
// UNLIMITED without changing the couple's reported tier ('premium' stays
// 'premium' everywhere else: billing status, custom-scripts/photos gates,
// UI badges). Fails CLOSED to false on DB error — the opposite stance from
// getCoupleTier, since this grants extra privilege rather than core access.
async function hasUnlimitedAI(coupleId) {
  if (!coupleId) return false;
  try {
    const result = await db.query(
      `SELECT 1 FROM couple_entitlements
        WHERE couple_id = $1 AND expires_at > NOW() AND unlimited_ai = TRUE
        LIMIT 1`,
      [coupleId]
    );
    return result.rows.length > 0;
  } catch (err) {
    logWarn('hasUnlimitedAI failed, defaulting to false', { err: err.message });
    return false;
  }
}
...
//
// limit can be UNLIMITED (premium) -> always ok. `unlimited: true` (the
// repo-owner unlimited_ai grant) bypasses the cap regardless of tier/key.
function checkLimit({ tier, key, used, unlimited }) {
