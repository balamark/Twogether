...
const { getCoupleIdForUser, getCoupleTier, getLimit, hasUnlimitedAI, UNLIMITED } = require('./entitlements');
...
const BILLABLE_KINDS = ['icebreaker', 'reply_rewrite', 'roleplay_messages', 'wall_counselor', 'reconciliation_opener', 'emotion_acceptance', 'marriage_checkup', 'story_insight', 'story_structure', 'need_translation', 'therapy_note', 'facilitation_turn', 'draft_analysis', 'closure_assist', 'closure_insight', 'deep_dive', 'therapy_topics'];
...
//
// unlimitedAi (from an unlimited_ai coupon grant, e.g. the repo-owner code)
// overrides the daily cap entirely — callers pass it through to checkLimit()
// as `unlimited` so the block never triggers, without changing `tier` itself.
async function resolveAiLimit(userId) {
  const coupleId = await getCoupleIdForUser(userId);
  const tier = await getCoupleTier(coupleId);
  const unlimitedAi = await hasUnlimitedAI(coupleId);
  const limit = unlimitedAi ? UNLIMITED : getLimit(tier, 'icebreaker_per_day');
  logInfo('ai.quota.resolve', { userId, coupleId, hasCouple: !!coupleId, tier, limit, unlimitedAi });
  return { tier, limit, coupleId, unlimitedAi };
}
